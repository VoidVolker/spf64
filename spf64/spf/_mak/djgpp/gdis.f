...
: 1_GD_STEP ( ADDR n -- ADDR1 )
   CR ."  .byte 0x"  0xFF AND  H. 1+ ;

: 2_GD_STEP ( ADDR -- ADDR1 )
  1_GD_STEP ." ,0x"  DUP C@ H. 1+ ;

: 3_GD_STEP ( ADDR -- ADDR1 )
  2_GD_STEP ." ,0x"  DUP C@ H. 1+ ;

: 4_GD_STEP ( ADDR -- ADDR1 )
  3_GD_STEP ." ,0x"  DUP C@ H. 1+ ;
...
: GN.  ( ADDR -- )
  ...
  IF ." 0x" H. EXIT THEN
    DUP WordByAddr  OVER >R GTYPE ." +0x"
  R> 1- NAME> - DUP TO #GN H. ;

: .INT ( ADDR -- ADDR1 )
 CR   DUP HSSSS @ + @ OVER @ = \ DROP 1
 IF   ."  .int 0x" DUP @ H.
 ELSE ."  .int "   DUP @ GN.
 THEN  CELL+ ;
...
\ 0111.0XXX
  ...
  DUP   0F3 = IF  1_GD_STEP  EXIT THEN  \
  ...
  DUP5B?1     IF  5_GD_STEP1 EXIT THEN
   ...
   DUP   0E9 = IF CR ."  .byte 0xE9"
                  CR ."  .int " 1A_GD_STEP ." -.-4" EXIT THEN  \  JMP
   DUP   0E8 = IF CR ."  call " 1A_GD_STEP 
                 DUP CELL- REL@ CELL+ 
                 DUP USER-VALUE-CODE =
                 OVER      USER-CODE = OR
                 IF CR ."  .int reserve+0x"
                    DROP DUP @ RESERVE -  H. CELL+
                       EXIT
                 THEN          
                 ...
                 IF  CR ." .byte 0x" DUP COUNT DUP>R DUP H.
                     CR ." .ascii " 1+ ATYPE
                     R> + 2+ EXIT
                 THEN          
                 DUP CELL- REL@ CELL+ 
...
\  DUP 0D2FF =   IF  2_GD_STEP   EXIT THEN  \ JMP  EDX
  ...
  DUP  873B =   IF CR ."  cmp " .EDI ." ,%eax" EXIT THEN  \ CMP EAX, X [EDI]
  DUP  878B =   IF CR ."  mov " .EDI ." ,%eax" EXIT THEN  \ MOV EAX, X [EDI]
  DUP  8F8B =   IF CR ."  mov " .EDI ." ,%ecx" EXIT THEN  \ MOV ECX, X [EDI]
  DUP  878D =   IF CR ."  lea " .EDI ." ,%eax" EXIT THEN  \ LEA EAX, X [EDI]
  DUP  87FF =   IF CR ."  incl " .EDI           EXIT THEN  \ INC   19F9 [EDI] 
  DUP  8788 =   IF CR ."  mov %al,"  .EDI      EXIT THEN  \ MOV     19F9 [EDI] , AL 
  DUP  8789 =   IF CR ."  mov %eax," .EDI      EXIT THEN  \ MOV X [EDI], EAX
  DUP  8703 =   IF CR ."  add %eax," .EDI      EXIT THEN  \ ADD EAX , X [EDI]
  DUP  8733 =   IF CR ."  xor %eax," .EDI      EXIT THEN  \ XOR EAX , X [EDI]
  DUP  87C7 =   IF CR ."  movl $0x" DROP DUP 6 + @
...
\ XXXX.X1XX 0100.0100 1000.1XX1
  ...
  DUP 87B60F = IF CR ."  movzx " DROP 1+ DUP .EDI ." ,%eax" EXIT THEN  \ MOVZX   EAX , BYTE PTR 19E3 [EDI] 
